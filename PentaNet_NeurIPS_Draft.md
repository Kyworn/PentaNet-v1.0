...
1. **Scaling behavior.** Experiments at 345M parameters (24 layers, 16 heads, 1024 embedding dim) on a single RTX 5080 (16 GB) reveal a reversal of the perplexity advantage. Under identical conditions (seed=42, batch_size=4, 10k iterations, WikiText-103), BitNet achieves PPL 273.0 while PentaNet reaches only PPL 320.1. Per-layer bucket analysis shows uniform ±2 occupancy loss across all layers (~22% vs the expected ~44% for a Gaussian distribution), ruling out depth-specific collapse. The root cause is the absmean scale: with 5 quantization levels, the denominator is too large for weights to reach the ±2 states, effectively reducing PentaNet to a noisy ternary system. A scale_factor ablation (multiplying absmean by 0.6, 0.8, 1.0) confirms this: sf=0.8 yields 34% ±2 occupancy (vs 22% at sf=1.0 and 47% at sf=0.6), but even the optimal scale_factor does not close the gap with BitNet at this scale.

2. **Scale factor sensitivity.** The pentanary space requires precise scale calibration that ternary does not. A "Short & Wide" architecture (12 layers × 1536 embed, 418M params) was tested to isolate depth from width effects. With sf=0.8, PentaNet achieves PPL 618 at 10k iterations — still far behind BitNet xlarge's PPL 273. The pentanary optimization landscape is fundamentally harder: too little outer-state usage loses resolution; too much saturates gradients.

3. **Kernel throughput.** The current Triton kernel implements zero-multiplier arithmetic at the source level but does not yet match cuBLAS throughput on GPU — modern BLAS libraries use tensor cores which make FADD and FMUL effectively equivalent in cost. A DeepShift-style kernel (Elhoushi et al., 2019) leveraging INT8 tensor cores, or a Split-K decomposition for large-$K$ shapes, would close this gap. The primary inference benefit on current GPU hardware is the 5.3× reduction in weight memory bandwidth.

4. **Single dataset.** While WikiText-103 is a standard benchmark, downstream task evaluation (MMLU, HellaSwag, ARC) is needed to confirm that lower perplexity translates into improved task performance.

5. **Comparison scope.** We compare against BitNet (ternary) only. A broader comparison including 2-bit PTQ methods (AQLM, QuIP#) and 4-bit QAT baselines would strengthen the positioning.

### 5.4 Future Work

- **Learnable per-layer scale factor.** The scale_factor ablation (Section 5.3) shows that a fixed multiplier of 0.8 improves ±2 utilization from 22% to 34%, but manual tuning is not scalable. A learnable scale parameter per layer, optimized jointly with weights, would automatically adapt the quantization grid to each layer's weight distribution.
- **Soft quantization alternatives.** The standard STE provides a zero-gradient approximation at quantization boundaries. Gumbel-softmax or differentiable soft quantization (e.g., temperature-scaled continuous relaxations) may provide richer gradient signal for the ±2 states.
- **Per-neuron scaling.** Replacing the global absmean scale with per-neuron or per-head scales would give the optimizer finer control over which weights occupy the $\pm 2$ states.
...
At 345M parameters, this advantage reverses: BitNet achieves PPL 273 versus PentaNet's PPL 320. Per-layer analysis reveals that the absmean scale is too coarse for 5-level quantization, yielding only 22% ±2 occupancy versus BitNet's 67% ±1 occupancy. A scale_factor ablation identifies sf=0.8 as optimal (34% ±2, balanced distribution), but even this does not close the gap. We report these findings transparently: PentaNet demonstrates that native pentanary quantization is feasible and beneficial in the capacity-starved regime (124M), but the current absmean + STE formulation does not scale. The pentanary optimization landscape is fundamentally richer but harder — unlocking its potential requires advances in adaptive scaling and gradient-aware training for multi-level discrete weight spaces.
