...
class QuantLinear(nn.Module):
    def __init__(
        self, in_features, out_features, bias=False, mode="pentanet", scale_factor=1.0
    ):
        super().__init__()
    ...
    def forward(self, x):
        scale = (self.weight.abs().mean() * self.scale_factor).clamp(min=1e-8)
...
class GPTConfig:
    def __init__(
        ...
        if size == "small":
            ...
            size == "short_wide"
        ):  # 12 layers × 1536 embed (~350M params) — test depth hypothesis
            self.n_layer = 12
            self.n_head = 12
            self.n_embd = 1536
...
def count_weight_distribution(model):
    ...
    for m in model.modules():
        if isinstance(m, QuantLinear):
            scale = (m.weight.abs().mean() * m.scale_factor).clamp(min=1e-8)
...
def count_weight_distribution_per_layer(model):
    """Count bucket occupancy per layer (for scaling diagnosis)."""
    layer_counts = {}
    layer_idx = 0
    for m in model.modules():
        if isinstance(m, QuantLinear):
            ...
            total = w_q.numel()
            for val in counts:
                counts[val] = (w_q == val).sum().item() / max(total, 1) * 100

            layer_counts[layer_idx] = {
                "shape": list(m.weight.shape),
                "scale": scale.item(),
                "buckets": counts,
                "pm2_occupancy": counts[-2] + counts[2],
            }
            layer_idx += 1
    return layer_counts
...
def train():
    parser = argparse.ArgumentParser(
        description="PentaGPT Training — Duel PentaNet vs BitNet"
    )
    parser.add_argument(
        "--size",
        type=str,
        default="large",
        choices=["small", "medium", "large", "xlarge", "short_wide"],
    )
    parser.add_argument(
        "--mode", type=str, default="pentanet", choices=["pentanet", "bitnet"]
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale multiplier for absmean (try 0.5-0.7 for PentaNet)",
    )
    ...
    train_loader = DataLoader(
        train_bin, args.batch_size, config.block_size, seed=args.seed
    )
    ...
    print(
        f"🚀 Training {mode_label} ({args.size}, seed={args.seed}) : {n_params / 1e6:.2f}M params on {device}"
    )
    ...
    if use_wandb:
        try:
            ...
            wandb.init(
                project="PentaNet-Scaling",
                name=f"{args.mode}-{args.size}-s{args.seed}",
                config={
                    "mode": args.mode,
                    "size": args.size,
    ...
    for it in range(args.max_iters):
        ...
        if it % args.eval_interval == 0 or it == args.max_iters - 1:
            ...
            gen_ids = model.generate(
                context, max_new_tokens=40, temperature=0.8, top_k=40
            )[0].tolist()
            ...
            if args.mode == "bitnet":
                print(
                    f"  Weights: {{-1: {w_counts[-1]}, 0: {w_counts[0]}, 1: {w_counts[1]}}}"
                )
            else:
                print(f"  Weights: {w_counts}")
            print(f'  Gen: "{gen_text}"')

            # Per-layer bucket analysis (for scaling diagnosis)
            layer_buckets = count_weight_distribution_per_layer(model)
    ...
    print(f"\n{'=' * 60}")
    print(
        f"🏁 FINISHED: {args.mode.upper()} ({args.size}, seed={args.seed}) — {n_params / 1e6:.2f}M params"
    )
    print(
        f"   Best val_loss: {best_val_loss:.4f}  |  PPL: {math.exp(best_val_loss) if best_val_loss < 20 else 'inf'}"
    )
